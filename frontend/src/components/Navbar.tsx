...
const Navbar = () => {
    ...
    return (
        <div className="w-full">
            ...
            {/* Main Header */}
            <header className="sticky top-0 z-50 bg-gradient-to-r from-blue-700 via-emerald-600 to-emerald-700 shadow-xl py-4">
                <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center gap-4 md:gap-8">
                    {/* Logo */}
                    <Link href="/" className="flex items-center gap-3 shrink-0 group">
                        <div className="w-12 h-12 rounded-xl overflow-hidden shadow-2xl bg-white flex items-center justify-center p-1 group-hover:scale-105 transition-transform duration-300">
                            <img src="/logo.png" alt="CloudsMall Logo" className="w-full h-full object-contain" />
                        </div>
                        <div className="hidden sm:block">
                            <span className="text-2xl md:text-3xl font-black text-white tracking-tighter">Clouds<span className="text-yellow-400 italic">Mall</span></span>
                            <p className="text-[10px] font-bold text-emerald-100/80 -mt-1 uppercase tracking-widest">Premium Shopping</p>
                        </div>
                    </Link>
                    ...
                    {/* Actions */}
                    <div className="flex items-center gap-2 md:gap-6 shrink-0 font-bold text-white">
                        <Link href="/cart" className="flex items-center gap-2 hover:bg-white/10 transition-all p-2 rounded-xl relative group">
                            ...
                            {cartCount > 0 && (
                                <span className="absolute top-0 left-0 bg-yellow-400 text-slate-900 text-[10px] font-black px-1.5 py-0.5 rounded-full ring-2 ring-emerald-600">
                                    {cartCount}
                                </span>
                            )}
                        </Link>
                        ...
                        <Link href={isAuthenticated ? "/profile" : "/login"} className="flex items-center gap-2 hover:bg-white/10 transition-all p-2 rounded-xl group">
                            <div className="w-10 h-10 bg-white/10 group-hover:bg-white/20 rounded-full flex items-center justify-center transition-all overflow-hidden border border-white/20">
                                {isAuthenticated ? (
